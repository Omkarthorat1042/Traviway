...
public interface PlaceRepository extends JpaRepository<Place, Integer> {
	...
	// Corrected JPQL query for case-insensitive search with optional parameters
	@Query("SELECT p FROM Place p WHERE " +
		   "(:title IS NULL OR LOWER(p.title) LIKE CONCAT('%', LOWER(:title), '%')) OR " +
		   "(:cityId IS NULL OR p.city.cityId = :cityId) OR " +
